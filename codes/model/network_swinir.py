...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, x_size):
        ...
        # else:
        #     G = self.window_size
        #     x_SDA = x_general.reshape(B, H // G, G, W // G, G, C).permute(0, 1, 3, 2, 4, 5)
        #     x_LDA = x_general.reshape(B, G, H // G, G, W // G, C).permute(0, 2, 4, 1, 3, 5)
        #     x_SDA = x_SDA.reshape(B * H * W // G ** 2, G ** 2, C)
        #     x_LDA = x_LDA.reshape(B * H * W // G ** 2, G ** 2, C)
        #
        #     # multi-head self-attention
        #     x_SDA = self.attncf(x_SDA, mask=self.attn_mask)  # nW*B, G*G, C
        #     x_LDA = self.attncf(x_LDA, mask=self.attn_mask)  # nW*B, G*G, C
        #     # ungroup embeddings
        #     x_SDA = x_SDA.reshape(B, H // G, W // G, G, G, C)
        #     x_LDA = x_LDA.reshape(B, H // G, W // G, G, G, C)
        #
        #     x_SDA = x_SDA.permute(0, 1, 3, 2, 4, 5).reshape(B, H, W, C)
        #     x_LDA = x_LDA.permute(0, 3, 1, 4, 2, 5).reshape(B, H, W, C)
        #
        #     x_SDA = x_SDA.view(B, H * W, C)
        #     x_LDA = x_LDA.view(B, H * W, C)
        #
        #
        # x = x_Swin + x_LDA
...
if __name__ == '__main__':
    ...
    model = SwinIR(upscale=4, img_size=(height, width),
                   ...
                   embed_dim=64, num_heads=[1, 1, 1, 1, 1, 1], mlp_ratio=2, upsampler='pixelshuffle')
    print(model)
