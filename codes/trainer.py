...
class Trainer():
    def __init__(self, args, loader, my_model, my_loss, ckp):
        ...
        self.device = torch.device("cuda:0" if not self.args.cpu else "cpu")
    ...
    def train(self):
        ...
        tb_writer.add_scalar(tags[0], self.error_last.detach().numpy(), epoch)
    ...
    def test(self):
        ...
        with torch.no_grad():
            for idx_scale, scale in enumerate(self.scale):
                    ...
                    if self.args.test_block:
                        ...
                        for iy in range(0, h, ip):
                            ...
                            for ix in range(0, w, ip):
                                ...
                                # add test_block
                                sr_p = utils.quantize(sr_p, self.args.rgb_range)
                                hr_p = hr[:, :, ty:ty + tp, tx:tx + tp]

                                save_list = [sr]
                                if not no_eval:

                                    sr_p = utils.torch_to_np(sr_p)
                                    hr_p = utils.torch_to_np(hr_p)
                                    ...
                                    if crop_border == 0:
                                        ...
                                        cropped_hr = hr_p[:, crop_border:-crop_border, crop_border:-crop_border, :]
                                        cropped_sr = sr_p[:, crop_border:-crop_border, crop_border:-crop_border, :]
                                    if self.args.test_metric is 'psnr':

                                        cropped_sr = (cropped_sr * 255.0).round().astype(np.uint8)  # float32 to uint8
                                        cropped_hr = (cropped_hr * 255.0).round().astype(np.uint8)  # float32 to uint8
                                        eval_acc += utils.calculate_psnr(cropped_sr, cropped_hr, 255.)
                ...
                psnr = eval_acc / img_num
                tb_writer.add_scalar(tags[1], psnr, epoch)
